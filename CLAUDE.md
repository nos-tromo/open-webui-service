...
A thin **deployment wrapper** for [Open WebUI](https://github.com/open-webui/open-webui) — no application source lives here. The entire repo is Docker Compose config (`docker/`) that runs a pinned upstream image (`ghcr.io/open-webui/open-webui`, pinned by tag **and** sha256 digest). Changes here are config/ops changes, not code changes.
